...
vector<char> terminals;			// For the terminals (alphabet) ofthe languages.
...
// Breadht First Traversal algorithm to find the terminals in the language.
int BFS(string state, string path)
{
	if (state.length() == 1)
	{
		cout << "YES" << endl;
		cout << path[0];// If  nonterminal then print all the  processed paths.
...
int main()
{
	...
	cout << "The terminals are:a,b" << endl;	//To print the terminals
	...
	cout << "The Nonn terminals are:S,X,Y" << endl;//To print the Non terminals
	...
	for (int i = 0; cin; i++)
	{
		...
		tempPair.second = second;// take the rule to the tempPair.second.
	...
	// Take the first element of the states vector to the currentState
...
// Control with BFS. If result is 1, then this word is for this language.
			{
				result = 1;
		...
		if (result == 0)
// If result is zero, this means that given word is not for this language.
		{
			cout << "NO" << endl;
		}
